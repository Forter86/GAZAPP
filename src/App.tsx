...
Дополнительная информация: ${formData.additionalInfo || 'Не указано'}`;
    ...
    try {
      ...
      // Используем URL бэкенда через localtunnel
      // Если VITE_API_URL не установлен или содержит placeholder, используем localtunnel URL
      let API_URL = import.meta.env.VITE_API_URL || 'https://silly-buttons-join.loca.lt/send-application';
      
      // Проверяем, что это не placeholder
      if (API_URL.includes('YOUR_BACKEND_NGROK_URL') || API_URL.includes('your_backend_ngrok_url')) {
        API_URL = 'https://silly-buttons-join.loca.lt/send-application'; // Используем localtunnel URL
      }
